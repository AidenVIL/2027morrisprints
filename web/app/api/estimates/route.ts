...
export async function POST(req: Request) {
  ...
  try {
    ...
    const res = await runEstimation(path, inventory_item_id)
    const grams = Number(res.grams)
    const timeSeconds = Number(res.timeSeconds)
    ...
    // Use pricing returned from pipeline if available
    const price = Math.round((res?.pricing?.final ?? 0) * 100)
    return NextResponse.json({ ok: true, grams, timeSeconds, price })
  } catch (e) {
